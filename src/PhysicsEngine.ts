...
export class PhysicsEngine {
  ...
  checkCollisions() {
    ...
    while (nodesToCheck.length > 0) {
      ...
      if (node2) {
        ...
        const [ball, ball2] = [node, node2] as [Circle, Circle]
  ...
  static simpleCircleCinnematic(ball: Circle, ball2: Circle) {
    // (100 * 10) / 10
    return (2 * ball2.mass * ball2.vx + ball.vx * (ball.mass - ball2.mass)) / (ball.mass + ball2.mass)
  }

  static fullCircleCinnematic(ball: Circle, ball2: Circle) {
